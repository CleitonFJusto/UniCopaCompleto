...
export default function Palpites() {
  ...
  async function carregarPalpites(uid) {
    ...
    if (!error && data) {
      ...
      // Preenche inputs com valores já salvos
      const inputs = {};
      data.forEach(p => {
        inputs[p.jogo_id] = {
          casa: String(p.gols_casa),
          fora: String(p.gols_fora)
        };
      });
      setPalpites(inputs);
    }
  ...
  // Palpites preenchidos e ainda não iniciados
              ...
              {palpitesParaRevisar.map(jogo => (
                <View key={jogo.id} style={styles.modalItem}>
                  <Text style={styles.modalJogo}>
                    {jogo.sigla_casa} vs {jogo.sigla_fora}
                  </Text>
...
const styles = StyleSheet.create({
  ...
  jogoCard: {
    ...
    borderRadius: 12, padding: 12, marginVertical: 4,
  ...
  inputGol: {
    backgroundColor: '#102030', borderWidth: 1, borderColor: '#1e2d3d',
    borderRadius: 8, color: 'white', fontWeight: 'bold', fontSize: 20,
  ...
  botaoCancelar: {
    flex: 1, backgroundColor: '#102030', borderRadius: 10,
    paddingVertical: 14, alignItems: 'center',
    borderWidth: 1, borderColor: '#1e2d3d'
  },
  textoBotaoCancelar: { color: 'white', fontWeight: 'bold' },
  botaoSalvar: {
    flex: 2, backgroundColor: '#f2cc2f',
